...
def downgrade():
    ...
    sa.Column('allocated_margin', sa.NUMERIC(), autoincrement=False, nullable=False),
    ...
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], name='trade_virtual_account_user_id_fkey', ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id', name='trade_virtual_account_pkey'),
    sa.UniqueConstraint('user_id', 'name', name='trade_virtual_account_user_id_name_key')
    )
    ...
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], name='trade_account_user_id_fkey', ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id', name='trade_account_pkey')
    )
    op.create_table('trade_order',
    sa.Column('id', sa.INTEGER(), autoincrement=True, nullable=False),
    sa.Column('user_id', sa.INTEGER(), autoincrement=False, nullable=False, comment='アクティブな注文などがあるため、RESTRICTとする。（現在はめんどいのでCASCADE）'),
    sa.Column('is_backtest', sa.BOOLEAN(), autoincrement=False, nullable=False),
    sa.Column('provider', sa.VARCHAR(length=255), autoincrement=False, nullable=False),
    sa.Column('market', sa.VARCHAR(length=255), autoincrement=False, nullable=False),
    sa.Column('product', sa.VARCHAR(length=255), autoincrement=False, nullable=False),
    sa.Column('periods', sa.INTEGER(), autoincrement=False, nullable=False),
    sa.Column('status', postgresql.ENUM('CANCELED', 'CANCELE_REQUESTED', 'READY', 'REQUESTED', 'CONTRACTED', name='positionstatus'), autoincrement=False, nullable=False),
    sa.Column('ask_or_bid', postgresql.ENUM('ASK', 'BID', name='askbid'), autoincrement=False, nullable=False),
    sa.Column('order_at', postgresql.TIMESTAMP(timezone=True), autoincrement=False, nullable=False),
    ...
    sa.Column('contract_at', postgresql.TIMESTAMP(timezone=True), autoincrement=False, nullable=True, comment='約定日時。Noneなら約定時に現在日時が入力される。バックテストのため、任意の日付を入力可能'),
    ...
    sa.Column('slippage', sa.NUMERIC(), autoincrement=False, nullable=True, comment='バックテストにおいては約定価格のブレを表現し、実際の取引ではショートサーキットのように動作します？？'),
    sa.Column('limit_price', sa.NUMERIC(), autoincrement=False, nullable=True, comment='指定価格で利益を確定します。'),
    sa.Column('loss_price', sa.NUMERIC(), autoincrement=False, nullable=True, comment='指定価格で損益を確定します。'),
    sa.Column('commission', sa.NUMERIC(), autoincrement=False, nullable=False),
    sa.Column('api_data', postgresql.JSON(astext_type=sa.Text()), autoincrement=False, nullable=False),
    ...
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], name='trade_order_user_id_fkey', ondelete='CASCADE'),
